...
<body class="text-neutral-900 pb-24 md:pb-0">
    ...
    <!-- CHECKOUT SAYFASI (Geri kalan kodunuz burada) -->
        ...
        <div class="bg-white rounded-3xl">
            ...
            <form onsubmit="app.submitOrder(event)" class="space-y-6">
                ...
                <div class="space-y-3 pt-2">
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center p-4 border rounded-xl cursor-pointer hover:bg-orange-50 transition-all has-[:checked]:border-orange-500 has-[:checked]:bg-orange-50 has-[:checked]:shadow-sm">
                            <input type="radio" name="payment_method" value="ideal" class="w-5 h-5 accent-orange-600" checked onchange="app.setPaymentMethod('ideal')">
                            <span class="ml-3 font-bold flex items-center gap-2"><i data-lucide="credit-card" class="w-5 h-5"></i> iDEAL (Online)</span>
                        </label>
                        <label class="flex items-center p-4 border rounded-xl cursor-pointer hover:bg-gray-50 transition-all has-[:checked]:border-orange-500 has-[:checked]:bg-orange-50 has-[:checked]:shadow-sm">
                            <input type="radio" name="payment_method" value="cash" class="w-5 h-5 accent-orange-600" onchange="app.setPaymentMethod('cash')">
                            <span class="ml-3 font-bold flex items-center gap-2"><i data-lucide="banknote" class="w-5 h-5"></i> Contant</span>
                        </label>
                        <label class="flex items-center p-4 border rounded-xl cursor-pointer hover:bg-gray-50 transition-all has-[:checked]:border-orange-500 has-[:checked]:bg-orange-50 has-[:checked]:shadow-sm">
                            <input type="radio" name="payment_method" value="card_at_door" class="w-5 h-5 accent-orange-600" onchange="app.setPaymentMethod('card_at_door')">
                            <span class="ml-3 font-bold flex items-center gap-2"><i data-lucide="credit-card" class="w-5 h-5"></i> Pin aan de deur</span>
                        </label>
                    </div>
                </div>
                ...
                <button type="submit" id="checkout-submit-btn" class="w-full bg-black text-white py-5 rounded-xl font-black text-lg shadow-xl hover:bg-gray-900 hover:scale-[1.01] active:scale-[0.99] transition-all flex justify-center items-center gap-3">
                    <span>Bestelling Plaatsen</span> <i data-lucide="arrow-right" class="w-5 h-5"></i>
                </button>
            </form>
        </div>
    </div>
    ...
    <!-- MODAL -->
    <div id="modal-product" class="hidden fixed inset-0 z-[70] flex items-end md:items-center justify-center p-0 md:p-4">
        ...
        <div class="relative w-full max-w-md bg-white rounded-t-3xl md:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div class="relative h-56 bg-gray-100 shrink-0">
                ...
                <button onclick="app.closeModal()" class="absolute top-4 right-4 bg-black/50 text-white p-2 rounded-full hover:bg-black backdrop-blur-md"><i data-lucide="x" class="w-5 h-5"></i></button>
                <div class="absolute bottom-6 left-6 text-white right-6">
                    <h3 id="modal-title" class="text-3xl font-black leading-none mb-2"></h3>
                    <p id="modal-price" class="opacity-90 font-bold text-xl bg-white/20 backdrop-blur-md inline-block px-3 py-1 rounded-lg"></p>
                </div>
            </div>
            <div class="p-6 overflow-y-auto">
                <h4 class="font-bold text-gray-900 mb-4 flex items-center gap-2"><i data-lucide="plus-circle" class="w-4 h-4"></i> Extra's toevoegen</h4>
                <div id="modal-extras" class="space-y-3"></div>
            </div>
            <div class="p-4 border-t bg-gray-50 shrink-0">
                <button onclick="app.addModalToCart()" id="modal-add-btn" class="w-full bg-orange-600 text-white py-4 rounded-xl font-black text-lg flex justify-between px-6 hover:bg-orange-700 shadow-lg shadow-orange-200 transition-all active:scale-[0.98]">
                    <span>Toevoegen</span>
        ...
        const app = {
            ...
            async submitOrder(e) {
                ...
                try {
                    ...
                    if (json.success) {
                        if (json.redirect_url) window.location.href = json.redirect_url; // Mollie
                        else alert("Bestelling geplaatst!");
                    } else {
                        alert(json.message || "Fout bij bestellen.");
