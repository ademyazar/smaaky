<?php
// Cache Buster: v_final_no_chdir_fix
...
// Eğer config.php bunu zaten yüklediyse, admin/_functions.php'yi SAKIN yükleme.
...
// 3. İŞLEMLER
try {
    // DÜZELTME: Mollie başlatma kodlarını sildik ama değişkeni null olarak tanımladık.
    // Böylece aşağıda kullanıldığında hata vermeyecek.
    $mollieClient = null;
    ...
    // Mollie Ödeme Başlatma
    if ($isMolliePayment && $mollieClient) {
        // Tamponu temizle (Mollie'ye gitmeden önce)
        if (ob_get_length()) ob_clean(); 
        ...
        if ($total <= 0.01) { throw new Exception("Tutar geçersiz."); }
        
        $payment = $mollieClient->createPayment(
            $total, 
            "Bestelling #" . $orderId, 
            "$baseUrl/order_success.php?order_id=" . $orderId, 
            "$baseUrl/api/mollie_webhook.php", 
            $orderId, 
            $paymentMethod 
        );
